...
public class modelDAO implements InterCRUD {
	@Override
	...
	public List<MUsuario> ConsultarTodos() {
		try {
			...
			PreparedStatement sentencia = conexion.preparedStatement(tx);
			ResultSet datos = sentencia.executeQuery();
			while(datos.next()) {
				MUsuario usuario = new MUsuario();
				usuario.setCedula_usuario(datos.getInt("cedula_usuario"));
	...
	public boolean Actualizar(MUsuario objeto) {
		if(objeto != null) {
			try {
				...
				sentencia.setInt(5, objeto.getCedula_usuario());
				sentencia.executeUpdate();
				desconectar();
				return true;
			}
